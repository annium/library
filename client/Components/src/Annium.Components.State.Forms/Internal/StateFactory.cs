...
namespace Annium.Components.State.Forms.Internal
{
    internal class StateFactory : IStateFactory
    {
        ...
        internal static MethodInfo ResolveFactory(Type type)
        {
            ...
            var inferCandidate = Factories.FirstOrDefault(xx => type.GetTargetImplementation(xx.GetParameters().Single().ParameterType) != null);
            if (inferCandidate is null)
        ...
        public IAtomicContainer<Guid> Create(Guid initialValue) => CreateAtomic(initialValue);
