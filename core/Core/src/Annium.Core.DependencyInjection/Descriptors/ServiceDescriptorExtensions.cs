    ...
    public static string ToReadableString(this IServiceDescriptor descriptor) =>
        descriptor switch
        {
            ITypeServiceDescriptor x
                => $"type {x.ServiceType.Namespace}.{x.ServiceType.FriendlyName()} -> {x.ImplementationType.Namespace}.{x.ImplementationType.FriendlyName()}",
            IFactoryServiceDescriptor x
                => $"factory {x.ServiceType.Namespace}.{x.ServiceType.FriendlyName()} ->  {x.ImplementationFactory.GetType().Namespace}.{x.ImplementationFactory.GetType().FriendlyName()}",
            IInstanceServiceDescriptor x
                => $"instance {x.ServiceType.Namespace}.{x.ServiceType.FriendlyName()} -> {x.ImplementationInstance.GetType().Namespace}.{x.ImplementationInstance.GetType().FriendlyName()}",
