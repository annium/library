...
/// </summary>
internal class BulkRegistrationBuilder : RegistrationBuilderBase, IBulkRegistrationBuilderBase
{
    /// <summary>
    ...
    /// <summary>
    /// Registers each type as each of its implemented interfaces with keys generated by the specified function.
    /// </summary>
    /// <param name="getKey">The function to generate keys for each (type, interface) registration.</param>
    /// <returns>The bulk registration builder target for method chaining.</returns>
    public IBulkRegistrationBuilderTarget AsKeyedInterfaces(Func<Type, object> getKey)
    {
        return WithRegistrations(type =>
            type.GetInterfaces().Select(x => new KeyedTypeRegistration(x, getKey(type), type))
        );
    }
    ...
    public IServiceContainer In(ServiceLifetime lifetime)
    {
        ...
        if (!_selfAdded)
            Registrations.AddRange(_types.Select(x => new TypeRegistration(x, x)));
    ...
    {
        Track(_types.SelectMany(createRegistrations));
        return this;
    }
    ...
    {
        Track(_types.Select(createRegistration));
        return this;
    }
}
