...
public static class Executor
{
    ...
    {
        public static IBackgroundExecutor Parallel<T>(ILogger logger) => new ParallelBackgroundExecutor<T>(logger);

        public static IBackgroundExecutor Concurrent<T>(ILogger logger, uint parallelism = 0) =>
            new ConcurrentBackgroundExecutor<T>(
                parallelism == 0u ? Environment.ProcessorCount : (int)parallelism,
                logger
            );

        public static IBackgroundExecutor Sequential<T>(ILogger logger) => new SequentialBackgroundExecutor<T>(logger);
    }
}
