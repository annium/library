...
{
    /// <summary>
    /// Creates a shell command instance for the specified command, split on spaces
    /// </summary>
    ...
    /// <summary>
    /// Creates a shell command instance from arguments passed verbatim
    /// </summary>
    /// <param name="args">The executable and its arguments</param>
    /// <returns>A configured shell instance for the command</returns>
    /// <exception cref="InvalidOperationException">Thrown when no executable is given</exception>
    public IShellInstance Cmd(params string[] args)
    {
        if (args.Length == 0)
