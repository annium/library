...
public class MessagingManagedSocketTests : TestBase, IAsyncLifetime
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="MessagingManagedSocketTests"/> class
    /// </summary>
    ...
    /// <summary>
    /// Tests sending extreme message that exceeds buffer size
    /// </summary>
    /// <param name="streamType">The type of stream to test</param>
    /// <returns>A task representing the test operation</returns>
    [Theory]
    ...
    public async Task Send_ExtremeMessage(StreamType streamType)
    {
        ...
        // act
        this.Trace("send message");
        await ManagedSocket.SendAsync(message, TestContext.Current.CancellationToken);
    ...
    /// <summary>
    /// Tests listening with extreme message that exceeds buffer size
    /// </summary>
    /// <param name="streamType">The type of stream to test</param>
    /// <returns>A task representing the test operation</returns>
    [Theory]
    ...
    public async Task Listen_ExtremeMessage(StreamType streamType)
    {
        ...
        var messages = GenerateMessages(1, 5 * 1024 * 1024);
        ...
        this.Trace("await listen result");
        var listenResult = await ListenAsync(TestContext.Current.CancellationToken);
    ...
    /// <param name="serverSocketOptions">Optional server socket options</param>
    ...
    /// <param name="server">Server, to connect to</param>
    /// <param name="ct">The cancellation token</param>
    /// <param name="options">Optional managed socket options</param>
    /// <returns>A task representing the connection operation</returns>
    private async Task ConnectAsync(
        IServer server,
        CancellationToken ct = default,
        ManagedSocketOptionsBase? options = null
    )
    {
        this.Trace("start");
        ...
        this.Trace("create managed socket");
        ManagedSocket = new MessagingManagedSocket(ClientStream, options ?? ManagedSocketOptionsBase.Default, Logger);
