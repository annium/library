...
public static class OrderExtensions
{
    ...
    public static bool IsActive<TOrder>(this TOrder order)
        where TOrder : IOrder
        =>
            order.Status is OrderStatus.New or OrderStatus.PartiallyFilled;

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool IsInactive<TOrder>(this TOrder order)
        where TOrder : IOrder
        =>
            order.Status is OrderStatus.Filled or OrderStatus.Canceled;

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool IsImmediate<TOrder>(this TOrder order)
        where TOrder : IOrder
        =>
            order.Type is OrderType.Limit or OrderType.Market;

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool IsLeveled<TOrder>(this TOrder order)
        where TOrder : IOrder
        =>
            order.Type is OrderType.StopLossMarket or OrderType.TakeProfitMarket;

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool IsLimit<TOrder>(this TOrder order)
        where TOrder : IOrder
        =>
            order.Type is OrderType.Limit;
    ...
    public static decimal CancellableQty<TOrder>(this TOrder order)
        where TOrder : IOrder
        =>
            order.Status is OrderStatus.Canceled ? order.TotalQty - order.ExecutedQty : 0;
}