...
{
    public static Result<MarketOperationStatus, T> Market<T>(T data) => new(MarketOperationStatus.Ok, data, string.Empty);
    public static Result<MarketOperationStatus, T> Market<T>(MarketOperationStatus status, T data, string error) => new(status, data, error);
    public static Result<MarketOperationStatus> Market() => new(MarketOperationStatus.Ok, string.Empty);
    public static Result<MarketOperationStatus> Market(MarketOperationStatus status, string error) => new(status, error);
    public static Result<UserOperationStatus, T> User<T>(T data) => new(UserOperationStatus.Ok, data, string.Empty);
    public static Result<UserOperationStatus, T> User<T>(UserOperationStatus status, T data, string error) => new(status, data, error);
    ...
    public static async Task<Result<MarketOperationStatus, T>> AsMarketAsync<T>(this Task<Result<UserOperationStatus, T>> task)
    {
    ...
    private static MarketOperationStatus MapStatus(UserOperationStatus x) => x switch
    {
        ...
        _                                 => throw new InvalidOperationException($"{nameof(UserOperationStatus)} {x} is not mapped to {nameof(MarketOperationStatus)}")
    };
};