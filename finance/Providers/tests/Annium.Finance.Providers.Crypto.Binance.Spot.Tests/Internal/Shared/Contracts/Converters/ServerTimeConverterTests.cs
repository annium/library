...
public class ServerTimeConverterTests : ProvidersTestBase
{
    ...
    public void Works()
    {
        ...
        var deserialized = serializer.Deserialize<Base.Shared.Contracts.Domain.ServerTime?>(Encoding.UTF8.GetBytes(raw)).NotNull();
    ...
    public void InvalidDataReturnsEmpty()
    {
        ...
        // act - deserialize
        var serializer = this.GetJsonSerializer(Constants.ServerTimeKey);
        var deserialized = serializer.Deserialize<Base.Shared.Contracts.Domain.ServerTime?>(Encoding.UTF8.GetBytes(raw));
