...
namespace Annium.Finance.Providers.Crypto.Binance.UsdFutures.Tests.Internal;
...
public class EndpointOverrideTests : ProvidersTestBase
{
    ...
    /// <summary>
    /// Registers the Binance USD-M futures provider with every endpoint pointed somewhere else.
    /// </summary>
    ...
    public void UnsetEndpoints_ResolveTheExchangesOwn()
    {
        ...
        // and the two constants they fall back to are distinct values, so the test above cannot pass by
        // both being the same string. There is no third: this market type serves the market stream and the
        // account stream from one host, which is why the override for each is separate even though their
        // defaults are not
