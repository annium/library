...
| **the futures account is unverified after a killed run** | the first spot trading run went through the recipe that selects the write block across the whole solution, which includes the other venue's trading tests, and it was killed part-way. Reading it back did not work - response bodies are not logged on read paths - so this is unproven rather than clear. Every run after it was filtered to one venue |
