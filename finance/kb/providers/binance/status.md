...
|---|---|
| **MIGRATE THE CONDITIONAL ORDER FAMILY TO `/fapi/v1/algoOrder`** — steps 3-5, specified 2026-09-18 | **Answered by the documentation pass; this is now implementation work, not a question.** The five types (`STOP_MARKET`, `TAKE_PROFIT_MARKET`, `STOP`, `TAKE_PROFIT`, `TRAILING_STOP_MARKET`) moved on 2025-12-09 and `POST /fapi/v1/order` refuses them with `-4120`. Full request shape, endpoint list and the three behavioural changes are in `manifest.md` §5, cited to the snapshot. **Not** `/sapi/v1/algo/futures/*`, which the error message's wording suggests and which is a different product. Shape of the work: step 3 gains converters for the algo responses and the `ALGO_UPDATE` event; step 4/5 route the four `OrderType`s through the new endpoints and re-map `algoId`/`clientAlgoId`. **One blocker of its own:** the response schemas are `unretrievable` from this snapshot, so the converters cannot be written from documentation alone — a logged live response is needed first, which makes a read-only probe the first task, not the last |
...
| `MapOperationCode` folds negative codes to `BadRequest` — **unblocked 2026-09-18, and sharper than it looked** | the reason it was deferred was that the real code list was missing and inventing a taxonomy would ship it to every caller, `crypted`'s order path included. `error-code.md` carries **206 codes in five families Binance groups itself** (`10xx` general/network, `11xx` request, `20xx` processing, `40xx` filters, `50xx` execution) — so the table is now a reading exercise, not a taxonomy exercise; the families are in `manifest.md` §6. And there is a **defect** behind it rather than only coarseness: `-1008 Request Throttled` falls to `BadRequest`, so a system-level throttle takes no pause in the limiter and the retry goes straight back into it. The ban-message regex cannot rescue it — that text states no deadline |
| ~~three `[UNVERIFIED]` markers~~ — **two resolved, one still open** | the rate-limit window **is** one minute (`common-definition.md:133-134`), so the hard-coded `-1m` header suffix is confirmed; the decay arithmetic resolved as **a copied constant** — 300/3000ms is spot's 6000/min put on futures' 2400/min ceiling, 2.5× too fast, with an XML doc claiming a match the arithmetic refuses. Still open: whether a one-way account reports one `positions[]` row per symbol with `positionSide=BOTH`. The documentation does not state it and the write fixture's precondition rests on it |
| **two defects of ours, found by comparison** — steps 4-5 | (1) futures decay should be `120` every 3000ms, not `300`; severity is bounded because the weight header overwrites the count on every response, but it biases the fallback toward permissiveness in exactly the situation the fallback exists for. (2) the post-refusal pause is capped at **1 hour** while documented bans run **2 minutes to 3 days** (`general-info.md:152`) — on a long ban we resume into it, which the same page names as the cause of longer bans. The second is the graver: it applies when the exchange has already refused us |
| **three silent changes to live behaviour** — step 4 | `GET /fapi/v1/userTrades` reach cut from 6 months to 3, so a back-fill past ~90 days returns empty pages indistinguishable from a quiet week; `allOrders` and `userTrades` now return **both** UM and CM rows, which we survive only by always sending `symbol` and which puts the `.Last()` paging cursor on a merged id sequence; and `MAX_NUM_ALGO_ORDERS` is `contested` between two pages of the same snapshot — settle it by logging one `exchangeInfo` response, which this module already fetches every 600 seconds |
