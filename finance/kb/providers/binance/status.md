...
- last reconciled: 2026-09-21 — **spot is level with futures, and every stage of it has been run
  live.** Steps 3, 4 and 5 converged offline, the trading block written, then validated against the
  venue: read, trading, lifetime and recovery, and finally two deliberate fills. Step 5 for spot had
  begun by finding that its account stream could not be ported - the key mechanism is deprecated and
  gone from the reference, and the replacement is a different transport, which is `manifest.md` §11,
  collected first. §11 now carries three `live` tables. The same day: the price band reached
  `InstrumentModel` on both market types, and the manifest's `[DEAD]` markers over the spot user path
  were lifted, a day after the path stopped being dead. Before it, on 2026-09-20: step 5 complete on
  futures, the write block clean three times, and a post-convergence defect fixed
...
| 2 — collect facts, compute drift | **converged, re-run 2026-09-18, with one accepted gap** | 13 futures pages, 7 spot files, both Postman collections, the per-endpoint API reference and one page reached by following a link out of the changelog. Every category given an outcome **with a citation into the snapshot** — the new rule, added to the skill during this run. Three mechanical sweeps ran before any reading by judgment | **one accepted gap**: the nested user-data-stream payloads (~20 short field names, now including `ALGO_UPDATE`'s) are `unretrievable` — they render from a schema embed present in neither index file. The `avgPrice` gap is **closed**, settled from documentation rather than by a live order. The one `contested` left, `MAX_NUM_ALGO_ORDERS`, was **settled live on 2026-09-19**: the payload carries no such filter at all, so the reference page is stale and there is nothing to implement |
...
| 4 — provider, read paths (+ registration, config, read-only live validation) | **converged on both venues; spot 2026-09-21** | every read path on both venues driven offline, failure paths included; endpoints pinned; **live read block green on 2026-09-16: 20 tests, 14 passed, 6 skipped, none failed** — and this time with credentials present, so the two signature tests ran rather than skipping | none. The upstream defect found here — an exchange error discarded when the success type is a collection — was fixed in `Annium.Net.Http` 1.1.49 and taken up with the package bump; the test that pinned the loss now pins the reason. Spot's `UserProviderTests` were the six skipped as `Not implemented`; the paths they drive were built on 2026-09-21 and they run now. The spot instrument now also carries the price band, which the read paths had been dropping ([report](2026.09/2026.09.21-instrument-price-band.md)) |
| 5 — connector, streams and orders (+ registration, config, trading live validation) | **converged 2026-09-20 on usd-futures; spot converged and validated live 2026-09-21** | the test lib gained a websocket server; the streams are pinned by 20 offline tests, the order lifecycle by 13 more, and every registration key by two. Six defects found and fixed in the process — see the [run report](2026.09/2026.09.18-step-5.md) | **none on spot's own path.** Offline: 35 tests, mutation-checked ([report](2026.09/2026.09.21-step-5-spot.md)); trading block built ([report](2026.09/2026.09.21-step-5-spot-trading-block.md)); **live: read 22/22 and trading 4/4 run twice, account clean after each** ([report](2026.09/2026.09.21-spot-live-validation.md)). Four facts in §11 are now `live`, and three defects were found getting there. ~~**Not asserted live: a fill**~~ - **closed 2026-09-21**: nothing in the block fills by design, so two round trips were run as a probe instead, one at market and one with a limit through the book ([capture](2026.09/2026.09.21-spot-fill/README.md), [report](2026.09/2026.09.21-spot-fill-probe.md)). Six facts moved to `live` in §11; the one gap it found - a null commission asset on the acceptance event - is pinned. ~~**Not verified: the futures account**~~ - the account holder looked on 2026-09-21 and reported both accounts clean. On usd-futures: none. **5f's fourth stage went green on 2026-09-19** once the conditional-order migration landed, and the whole write block then ran end to end **three times clean** - 8 of 8 each time, no connector errors, the account flat after each, and the three runs identical in what they traded. See [the run report](2026.09/2026.09.19-trading-runs.md). **5e validated live on 2026-09-18**: 21 tests, 15 passed, 6 skipped, none failed, run twice - including a new read-only user connector test that connects, takes a listen key, opens the stream and receives the account snapshot without placing anything. The two behaviours that needed a decision have one: a refused leverage change now reaches the caller, and one the exchange accepts without applying is refused by the connector itself; a limit order asked to become a market order stays refused |
...
**Validated live the same day.** Read 22/22, trading 4/4 run twice, the account left with nothing open
after each - see [the live report](2026.09/2026.09.21-spot-live-validation.md). It took three defects and
one test that passed for the wrong reason: a scheduled reload that spent the whole weight allowance on
the one list the contract had already warned about; a cancel-all whose answer is the list of orders it
cancelled and was read as an acknowledgement envelope, so a cancellation that had happened was reported
as a failure; a cancel-all refused on an empty book under a code whose meaning is in its message; and a
resting price chosen far enough from the market to be refused for being far.
...
is bought, sold or charged. A fill is not asserted live in the block, and that is a decision rather than
an oversight — **it was settled separately on 2026-09-21 by a probe** that filled twice on purpose, at
market and with a limit through the book, and recorded every answer
([capture](2026.09/2026.09.21-spot-fill/README.md),
[report](2026.09/2026.09.21-spot-fill-probe.md)). The block stays as it is: a suite that fills on every
run pays a fee on every run and needs the account to be tradeable rather than merely funded.
...
| 2026-09-21 | 5 — live validation (spot) | [`2026.09/2026.09.21-spot-live-validation.md`](2026.09/2026.09.21-spot-live-validation.md) | **read 22/22, trading 4/4 run twice, account clean after each.** The connector reached connected against the real account for the first time, which on this venue means a signed WebSocket API subscription was accepted - the new transport works, with an ordinary HMAC key. Four fixes on the way, and three were defects. **The contract predicted the first**: §10 had written down that an unscoped open-order list costs 80 and a one-second reload of it is 4800 a minute against a ceiling of 6000, and the live test config set exactly that, copied from the venue where it costs half as much - a fact recorded and ignored in configuration protects nothing. **The worst was the second**: cancel-all answers the array of orders it cancelled, was read as the other venue's `{code,msg}` envelope, and so reported a failure for a cancellation that had already happened. **The third**: cancel-all is refused on an empty book under a code that also carries "market is closed" and "this account may not place or cancel orders", so it cannot be folded into success - the fixture looks before it asks. **The fourth was not a defect but a test passing for the wrong reason**: the minimum-notional refusal passed while the venue was refusing every order in the block for an unrelated price filter. Finance 576 → 577 |
| 2026-09-21 | 5 — live: lifetime and recovery (spot) | *(no report; the work is in [library#42](https://github.com/annium/library/pull/42))* | **the two things no short live test could reach.** A hold test keeps the connection for 150s, past the minute a venue allows a connection that answers nothing - it survived, so the keep-alive works end to end, and the answer being the framework's rather than ours is now observed rather than reasoned about. A byte relay lets a test cut a live connection: the stream noticed, reconnected, and **the venue accepted a second signed subscription on the new connection**. The relay is not a WebSocket proxy on purpose - one that understood the protocol would answer the venue's ping itself and hide what the hold test exists to measure. It also gives the hold test the positive control it lacked: asserting a transition never happened cannot distinguish that from nothing watching, and now the same signal has been made to fire. Read block 22 → 24 |
| 2026-09-21 | 5 — live: lifetime and recovery (usd-futures) | *(no report; the work is in [library#42](https://github.com/annium/library/pull/42))* | **the same two instruments pointed at the other venue, and it is the more interesting of the two.** Its stream is opened on a key fetched over REST and refreshed on a timer rather than kept alive by answering a protocol ping, so what can go wrong differs - a refresh that stops, or one the venue declines - while the symptom is identical: a connector that reconnects forever and from outside looks like one that works. The hold outlives the refresh interval, not the key's own lifetime, which is beyond what a suite can wait for. The drop test measures the chain no offline test vouches for: socket cut, resolver asked for another key, venue issues one, second socket opened on it - offline the resolver is driven by hand, so every link there is assumed. Only the socket goes through the relay; the resolver talks to the venue directly, and the relay forwards the request line as sent so the key in the path travels untouched. Read block 24 → 26 |
| 2026-09-21 | 3/4 — filters and model | [`2026.09/2026.09.21-instrument-price-band.md`](2026.09/2026.09.21-instrument-price-band.md) | the price band reaches `InstrumentModel` on both market types, closing the gap the spot trading block's first live run opened and one backlog item that had been open longer. The divergence was deeper than the field names: one venue states both ends per side, the other one inequality per side, and the tidy reading of the second would make the provider refuse orders the exchange accepts - refuted by our own futures runs rather than by argument. A live census of both public payloads settled the absent-filter question: universal today, read as optional anyway, because an unbounded band is a statement where a missing price filter is not. Seven mutants, six dead; the survivor found a guard that could never fire. Offline 594/594 |
| 2026-09-21 | 5 — live: execution (spot) | [`2026.09/2026.09.21-spot-fill-probe.md`](2026.09/2026.09.21-spot-fill-probe.md) | **the first fills on this market type.** Its trading block prices every order away from the market, so the events an execution raises had been documented, transcribed, pinned - and never seen. Two real round trips, approved one at a time and run as a probe: market, then a limit through the book. Six facts to `live`, three of them confirmations that an existing transcription was right, which is what a capture is for. One real gap: the acceptance event carries a null commission asset and a bare `"0"` amount, spelled otherwise in every hand-written fixture in the suite - now pinned, three mutants dead. Cost 0.04 USDT and a dust remainder below the lot step. Offline 596/596 |
| 2026-09-21 | 4-5 — both live blocks, both venues, three runs each | *(no report; the runs are recorded here)* | run after the price band changed how instruments are parsed on both venues - the only change since the last live pass that touches a path these blocks exercise. **Read: 13/13 on each venue, three times each, six runs, nothing failed.** **Write: spot 5/5 three times, futures 8/8 three times, nothing failed.** Accounts read before and after: spot ends at the cent it started at, because its block rests every order and fills none; futures ends flat with the wallet 0.0098 USDT higher than it began, three round trips' fees included, which is the market moving rather than anything about the code. Nothing to fix, so nothing was re-run |
...
| ~~the filter converters have no test file~~ — **closed 2026-09-21** | both venues have one now, written with the price band: `InstrumentFiltersConverterTests`, driving the `filters` array directly. That is what the file was for - a case costs a dozen lines instead of a hundred, which is what made it affordable to ask for a band whose four ends are four different numbers and for an array missing one |
| ~~**`PERCENT_PRICE_BY_SIDE` is not in `InstrumentModel`**~~ — **closed 2026-09-21** | it is, as four ratios of the venue's reference price, on both market types, with zero keeping its established meaning of an end the venue does not bound. The divergence turned out to be more than field names - one venue states both ends per side, the other one inequality per side - and reading the second as a two-sided band would have this provider refuse orders the exchange takes, which our own futures runs demonstrate. See [the report](2026.09/2026.09.21-instrument-price-band.md). Originally measured live 2026-09-21: the venue bounds how far from the market a limit order may be priced, against an average of recent trades rather than the current price, and the provider reads every other filter into the instrument and drops this one. So a caller pricing away from the market cannot know how far it may go, and learns by a refusal that names a filter rather than a bound. Step 3 and 4 work - the filter converter and the model both change |
| ~~**connection lifetime and recovery are untested**~~ — **closed 2026-09-21** | two instruments, because one could not do both. A **hold test** sits on the connection for two and a half minutes, past the deadline a venue enforces by pinging and closing what does not answer - every other live test here finishes in fifteen seconds and would never know. And a **byte relay** the test can cut, which is deliberately not a WebSocket proxy: a proxy that understood the protocol would answer the venue's ping itself and mask the very thing the long test measures. The relay also settles a doubt the hold test could not settle about itself - it asserts a transition never happened, and only a transition made to happen on purpose shows that anything was watching |
| ~~**the futures account is unverified after a killed run**~~ — **closed 2026-09-21** | the account holder looked and reported both accounts clean. The reason it could not be read back from here stands as its own small finding: response bodies are not logged on read paths, so a check that looks like one proves nothing |
| ~~spot's account-stream subscription spends weight nothing counts~~ — **closed 2026-09-21** | the transport has no rate-limit header, so the limiter learned nothing from it: 2 on connect and 2 per retry against a 6000/min ceiling, invisible. The reply carries its own accounting in a `rateLimits` array, and `WsApiUserStream.ReportUsedWeight` now feeds the request-weight entry to the limiter. A reply that states nothing leaves the limiter untouched rather than counting zero - a different claim, and both are pinned offline |
| ~~spot is a step behind~~ — **closed 2026-09-21** | it is not. Steps 3, 4 and 5 are converged offline and validated live: read 24/24, trading 5/5 including the sell side, lifetime and recovery measured. What is left is listed above and is the same kind of work the other venue also has left |
