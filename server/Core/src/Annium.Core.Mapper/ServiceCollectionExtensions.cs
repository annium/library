...
namespace Annium.Core.DependencyInjection
{
    public static class ServiceCollectionExtensions
    {
            ...
            if (profileType.IsGenericType)
            {
                ...
                foreach (var argument in profileType.GetGenericArguments())
                {
                    if (argument.GetGenericParameterConstraints().Length == 0)
                        throw new ArgumentException($"Can't add generic Profile {profileType} with unconstrained parameter {argument}");
                    var implementations = TypeManager.Instance.Types
        ...
        public static IServiceCollection AddMapper(this IServiceCollection services, bool autoload = true)
        {
            ...
            if (autoload)
            {
                foreach (var profile in TypeManager.Instance.GetImplementations(typeof(Profile)))
