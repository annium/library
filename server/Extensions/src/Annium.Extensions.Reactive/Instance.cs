...
namespace System
{
    ...
    {
        public static IObservableInstance<T> Dynamic<T>(Func<ObserverContext<T>, Task> factory)
        {
            return new DynamicObservableInstance<T>(factory);
        }

        public static IObservableInstance<T> Static<T>(Func<ObserverContext<T>, Task> factory)
        {
